...
def get_gnn(args, training=True):
    ...
    # build model
    model = GNNStack(   1, args.edge_trans_dim-1,  #node feature nums    #edge feature nums
...
class GNNStack(torch.nn.Module):
    ...
    def forward(self, x, edge_attr, edge_index, training=True):
        self.x = x
        #GNN starting
        ...
        for l,(conv_name,conv) in enumerate(zip(self.model_types,self.convs)):
            if conv_name == 'EGCN' or conv_name == 'EGSAGE':
                self.x = conv(self.x, edge_attr, edge_index)
            else:
                self.x = conv(self.x, edge_index)
            ## edge update
            if self.concat_states:
                concat_x.append(x)
            edge_attr = self.update_edge_attr(self.x, edge_attr, edge_index, self.edge_update_mlps[l])

        return self.x, edge_attr 
